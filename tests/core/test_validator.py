...
class TestUsdValidator(unittest.TestCase):
    ...
    def test_find_latest_department_version_success(self):
        ...
        # Act
        result = self.validator._find_latest_department_version(
            mock_dir, "assetA", "model"
        )
    ...
    def test_validate_department_layer_missing_root_prim(self, mock_find_or_open):
        ...
        # Act
        self.validator._validate_department_layer(
            Path("/fake/path.usd"), "/assetA", "model", "geo"
        )

        # Assert
        self.assertTrue(
            any(
                "Required root prim is missing" in err for err in self.validator._errors
            )
        )
    ...
    def test_validate_model_structure_forbidden_element(self, mock_find_or_open):
        ...
        # Assert
        self.assertTrue(
            any("Forbidden element 'Material'" in err for err in self.validator._errors)
        )
    ...
    def test_validate_model_structure_mesh_no_vertices(self, mock_find_or_open):
        ...
        # Assert
        self.assertTrue(
            any("Geo mesh with no vertices" in err for err in self.validator._errors)
        )
    ...
    def test_validate_root_invalid_kind(self, mock_find_or_open, mock_tokens):
        ...
        # Assert
        self.assertTrue(
            any("missing or invalid 'kind'" in err for err in self.validator._errors)
        )

    @patch("pxr.UsdGeom.PrimvarsAPI")
    @patch("pxr.Usd.Stage.Open")
    def test_validate_stage_content_missing_uvs_warning(
        self, mock_stage_open, mock_primvars_api_cls
    ):
        # Arrange
        ...
        with self.assertLogs("src", level="WARNING") as log_capture:
            ...
            self.assertTrue(
                any(
                    "does not contain texture coordinates" in msg
